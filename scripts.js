...
//         data.results.map(employee => console.log(employee.name) );
...
//             const timeZone = `${employee.location.timezone.offset}`;
//             const profilePicture = `${employee.picture.large}`;

//             const phoneNum = `${employee.phone}`;
//             const streetName = `${employee.location.street.name}`;
//             const addressNum = `${employee.location.street.number}`;
...
//                 people.push(employee) //Code Advised by @Emma W to create separate Array for PREV/NEXT
...
function employeeCard(employees){
        ...
        employees.map(employee => { //employee.name.first
